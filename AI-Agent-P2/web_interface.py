    ...
    if st.button("🔄 Refresh Reflections"):
        ...
        for line in raw.split("\n"):
            if "[Reflection]" in line:
                ...
                # Continue adding lines to the current reflection block
                if current_block:
                    current_block.append(line)
...
user_input = st.chat_input("Ask about Junos, EVPN, BGP, SRX, RIB, FPC etc...")

# Render previous messages
with chat_window:
    for msg in st.session_state.chat_history:
        # Assistant uses new avatar
...
if user_input:
    ...
    rag_list = retrieve_context(user_input)
    st.session_state.last_pdf_refs = rag_list

    with st.spinner("Thinking like a Juniper TAC engineer…"):
        answer = chat_with_agent(user_input)

    # Save assistant message
    st.session_state.chat_history.append({"role": "assistant", "content": answer})

    # Assistant uses NEW Fav.png icon
    st.chat_message(
        "assistant",
        avatar=ASSISTANT_AVATAR if os.path.exists(ASSISTANT_AVATAR) else "💡"
    ).markdown(answer)

    st.success("Response stored!")
